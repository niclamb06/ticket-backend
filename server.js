...
// ========== PostgreSQL Connection (mit Reconnect & Keep-Alive) ==========
const pool = new Pool({
  connectionString: process.env.DATABASE_URL,
  ssl: process.env.NODE_ENV === 'production' ? { rejectUnauthorized: false } : false,
  max: 5, // Connection Pool
  idleTimeoutMillis: 30000, // inaktive Verbindungen nach 30s schließen
  connectionTimeoutMillis: 5000 // max. Wartezeit für neue Verbindung
});

// Verbindung testen & automatisch reconnecten
async function connectWithRetry() {
  try {
    await pool.query('SELECT 1');
    console.log('✅ Verbindung zu PostgreSQL erfolgreich');
  } catch (err) {
    console.error('⚠️ Verbindung fehlgeschlagen, neuer Versuch in 5s...', err.message);
    setTimeout(connectWithRetry, 5000);
  }
}
connectWithRetry();

// Keep-Alive: alle 5 Minuten ping an die DB
setInterval(async () => {
  try {
    await pool.query('SELECT 1');
    console.log('🔄 Keep-Alive Ping erfolgreich');
  } catch (err) {
    console.error('Ping fehlgeschlagen:', err.message);
  }
}, 5 * 60 * 1000);
...
app.post('/api/groups', async (req, res) => {
  try {
    ...
    if (error.code === '23505') return res.status(400).json({ error: 'Gruppe existiert bereits' });
    console.error(error);
...
app.post('/api/authors', async (req, res) => {
  try {
    ...
    if (error.code === '23505') return res.status(400).json({ error: 'Autor existiert bereits' });
    console.error(error);
...
app.put('/api/admin/password', async (req, res) => {
  try {
    ...
    if (oldPassword === currentPassword) {
      await pool.query("UPDATE settings SET value = $1 WHERE key = 'adminPassword'", [newPassword]);
